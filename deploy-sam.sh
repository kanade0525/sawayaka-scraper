...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "⚠️  注意: 既存のEventBridgeルールを使用する場合は、"
    echo "    template.yamlのSourceArnを既存ルールのARNに更新してください"
    echo ""
    echo "🔍 関数の確認方法:"
    ...
    echo ""
    echo "📁 CSVファイルの確認方法:"
    echo "   aws s3 ls s3://sawayaka-scraper-data-{ACCOUNT_ID}/"
    echo ""
    echo "💾 機能:"
    echo "   - さわやか全店舗の待ち時間情報をスクレイピング"
    echo "   - CSVファイルとして自動生成"
    echo "   - S3バケットに自動アップロード"
    echo "   - 既存のEventBridgeルールで実行"
else
    echo "❌ SAMデプロイが失敗しました"
    exit 1
fi
