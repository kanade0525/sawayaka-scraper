        ...
        const shopData = await page.evaluate(() => {
            ...
            shopElements.forEach((shopElement, index) => {
                try {
                    ...
                    const shopName = shopNameElement ? shopNameElement.textContent.trim() : '店舗名不明';
                    
                    // 待ち時間を取得
                    const waitTimeElement = shopElement.querySelector('.wait_time .time .num');
                    const waitTime = waitTimeElement ? waitTimeElement.textContent.trim() : '--';
                    
                    // 待ち組数を取得
                    const waitCountElement = shopElement.querySelector('.wait_time .set .num');
                    const waitCount = waitCountElement ? waitCountElement.textContent.trim() : '--';
